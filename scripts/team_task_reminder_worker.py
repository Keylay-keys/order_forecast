#!/usr/bin/env python3
"""Team Task reminder worker.

Runs as a bounded --once CronJob. It claims due reminders transactionally,
sends one assignee notification, and records the result on the task document.
"""
...
WORKER_ID = f"team-task-reminder-{socket.gethostname()}-{os.getpid()}"
TEAM_TASK_REMINDERS_ENABLED = os.environ.get("TEAM_TASK_REMINDERS_ENABLED", "true").lower() == "true"
TEAM_TASK_REMINDER_DRY_RUN = os.environ.get("TEAM_TASK_REMINDER_DRY_RUN", "false").lower() == "true"
TEAM_TASK_REMINDER_BATCH_LIMIT = int(os.environ.get("TEAM_TASK_REMINDER_BATCH_LIMIT", "100"))
TEAM_TASK_REMINDER_CLAIM_TTL_MS = int(os.environ.get("TEAM_TASK_REMINDER_CLAIM_TTL_MS", str(10 * 60 * 1000)))
TEAM_TASK_REMINDER_DUE_GRACE_MS = int(os.environ.get("TEAM_TASK_REMINDER_DUE_GRACE_MS", str(2 * 60 * 1000)))

logger = logging.getLogger("team_task_reminder_worker")
...
def _format_due(data: Dict[str, Any]) -> str:
    due_date = str(data.get("dueDate") or "").strip()
    due_time = data.get("dueTime") if isinstance(data.get("dueTime"), dict) else None
    if due_time:
        minute = int(due_time.get("minute") or 0)
        return f"{due_date} {due_time.get('hour')}:{minute:02d} {due_time.get('period')}"
    return due_date
...
def _task_ref_from_snapshot(doc: Any) -> Any:
    ref = getattr(doc, "reference", None)
    if ref is None:
        raise RuntimeError("Team task snapshot is missing reference")
    return ref


def claim_task_reminder(db: firestore.Client, doc: Any, *, now_ms: int) -> Optional[Dict[str, Any]]:
    task_ref = _task_ref_from_snapshot(doc)
    ...
    def _claim(transaction):
        ...
        due_at_ms = _to_millis(data.get("dueAtMs"))
        ...
        is_stale_sending = reminder_status == "sending" and claimed_at_ms <= now_ms - TEAM_TASK_REMINDER_CLAIM_TTL_MS
        if status != "open" or not bool(data.get("reminderEnabled")):
            return None
        if not due_at_ms or now_ms > due_at_ms + TEAM_TASK_REMINDER_DUE_GRACE_MS:
            transaction.update(
                task_ref,
                {
                    "reminderStatus": "skipped",
                    "reminderSkippedAtMs": now_ms,
                    "reminderSkipReason": "past_due",
        ...
        if not (is_pending_due or is_stale_sending):
            return None
        if not data.get("teamMemberUid"):
            transaction.update(
                task_ref,
                {
                    "reminderStatus": "skipped",
                    "reminderSkippedAtMs": now_ms,
                    "reminderSkipReason": "missing_assignee",
                    "updatedAt": firestore.SERVER_TIMESTAMP,
                },
            )
            return None
        ...
        return {"ref": task_ref, "task": claimed}

    return _claim(db.transaction())


def finish_task_reminder(
    task_ref: Any,
    *,
    now_ms: int,
    sent: bool,
...
def send_claimed_reminder(db: firestore.Client, claimed: Dict[str, Any], *, now_ms: int) -> bool:
    ...
    task = claimed["task"]
    uid = str(task.get("teamMemberUid") or "")
    user_data = _user_doc_data(db, uid)
    title = "Team task reminder"
    body = _reminder_body(task)
    payload = {
        "routeNumber": str(task.get("routeNumber") or ""),
        "taskId": str(task.get("id") or ""),
        "target": "teamTasks",
    }

    if TEAM_TASK_REMINDER_DRY_RUN:
        logger.info("[dry-run] Would send team task reminder route=%s task=%s uid=%s", payload["routeNumber"], payload["taskId"], uid)
        finish_task_reminder(task_ref, now_ms=now_ms, sent=True, push_stats={"sent": 0, "failed": 0, "dryRun": 1})
        return True
    ...
    try:
        _write_user_notification(
            ...
            title=title,
            body=body,
            notification_type="team_task_reminder",
            data=payload,
        )
        push_stats = _send_expo_push(
            ...
            title=title,
            body=body,
            data={"type": "team_task_reminder", **payload},
        )
        sent = push_stats.get("sent", 0) > 0
        finish_task_reminder(task_ref, now_ms=now_ms, sent=sent, push_stats=push_stats)
        return sent
    except Exception as exc:
        logger.exception("Failed sending team task reminder")
        finish_task_reminder(task_ref, now_ms=now_ms, sent=False, push_stats={"sent": 0, "failed": 1}, error=str(exc))
        return False


def run_once(db: firestore.Client, *, now_ms: Optional[int] = None, limit: int = TEAM_TASK_REMINDER_BATCH_LIMIT) -> Dict[str, int]:
    if not TEAM_TASK_REMINDERS_ENABLED:
        logger.info("TEAM_TASK_REMINDERS_ENABLED=false; skipping")
        return {"claimed": 0, "sent": 0, "failed": 0, "skipped": 0}

    now = now_ms or _now_ms()
    cutoff = now - TEAM_TASK_REMINDER_CLAIM_TTL_MS
    ...
    for doc in docs:
        ...
        if doc_key in seen:
            continue
        seen.add(doc_key)
        claimed = claim_task_reminder(db, doc, now_ms=now)
        if not claimed:
            stats["skipped"] += 1
            continue
        stats["claimed"] += 1
...
def run_worker(sa_path: str) -> Dict[str, int]:
    ...
    stats = run_once(db)
    logger.info("Team task reminder worker stats: %s", stats)
    return stats


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Team Task reminder worker")
