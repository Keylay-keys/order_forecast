...
def _rows(products: Iterable[Dict[str, Any]], *, catalog_id: str, source_label: str) -> List[tuple]:
    rows: List[tuple] = []
...
def load_reference_catalog(path: Path, *, catalog_id: str = DEFAULT_CATALOG_ID, source_label: str = "repo") -> int:
    products = json.loads(path.read_text())
    if not isinstance(products, list):
        raise ValueError(f"Expected a JSON array in {path}")

    rows = _rows(products, catalog_id=catalog_id, source_label=source_label)
    conn = get_connection()
    try:
        create_schema(conn)
...
def main() -> None:
    ...
    count = load_reference_catalog(args.source, catalog_id=args.catalog_id, source_label=args.source_label)
